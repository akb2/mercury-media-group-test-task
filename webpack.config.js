...
const fixPath = src => path
  .resolve(path.normalize(src))
  .replace(/[\\]+/g, "/");
...
module.exports = (env, option) => {
  ...
  // Поиск страниц PUG
  {
    const pages = glob.sync(fixPath(__dirname + '/' + config.folders.input.pages + '/**/*.pug'));

    pages.forEach(function (file) {
      const base = path
        .relative(__dirname + '/' + config.folders.input.pages, file)
        .replace(/\.pug$/, '');
      const template = fixPath(config.folders.input.pages + '/' + base + '.pug');
      const filename = fixPath(config.folders.output.base + config.folders.output.pages + '/' + base + '.html');
